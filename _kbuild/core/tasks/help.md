...
1. **Load catalog** — Load `{project-root}/_kbuild/_config/kBuild-help.csv`

2. **Resolve output locations** — Scan each folder under `_kbuild/` (except `_config`) for `config.yaml`. For each workflow row, resolve its `output-location` variables against that module's config so artifact paths can be searched.
...
## RECOMMENDED OUTPUT FORMAT
   ...
   **Required items next** — List the next required workflow
   For each item show:
   - Workflow **name**
   - **Command** (prefixed with `/`, e.g., `/kBuild:example:build-prototype`)
