...
| ------------ | ------------------------- | ---------------------------------------------- |
| `id`         | Compiled output path      | `_kbuild/agents/{agent-name}/{agent-name}.md`    |
...
| `module`     | Team/ecosystem membership | `stand-alone`, `kbuild`, `cis`, `kbgd`, or custom |
...
**Format:** `_kbuild/agents/{agent-name}/{agent-name}.md`

**Examples:**
```yaml
id: _kbuild/agents/commit-poet/commit-poet.md
id: _kbuild/agents/journal-keeper/journal-keeper.md
id: _kbuild/agents/security-engineer/security-engineer.md
```
...
| `kbuild`         | Business Management Module              |
| `cis`         | Continuous Innovation System            |
| `kbgd`        | KBUILD Game Development                   |
...
| What's the unique key?     | `id` - `_kbuild/agents/commit-message-artisan/commit-message-artisan.md` (future lookup) |
...
```yaml
id: _kbuild/agents/different-agent/different-agent.md
```

**Fix:** The `id` must match the filename:
```yaml
id: _kbuild/agents/my-agent/my-agent.md
```
