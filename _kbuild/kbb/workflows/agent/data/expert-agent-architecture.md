...
During KBUILD INSTALLATION, sidecar folder is copied from the agent location to `{project-root}/_kbuild/_memory/{sidecar-folder}/`

**ALL agent YAML references MUST use:**

```yaml
{project-root}/_kbuild/_memory/{sidecar-folder}/{file}
```
...
# ✅ CORRECT
critical_actions:
  - "Load COMPLETE file {project-root}/_kbuild/_memory/journal-keeper-sidecar/memories.md"
  - "ONLY read/write files in {project-root}/_kbuild/_memory/journal-keeper-sidecar/"

menu:
  - action: "Update {project-root}/_kbuild/_memory/journal-keeper-sidecar/memories.md with insights"
```
...
agent:
  metadata:
    id: _kbuild/agents/{agent-name}/{agent-name}.md
    name: 'Persona Name'
    title: 'Agent Title'
    icon: '🔧'
    module: stand-alone           # or: kbuild, cis, kbgd, other
  ...
  critical_actions:
    - 'Load COMPLETE file {project-root}/_kbuild/_memory/{sidecar-folder}/memories.md'
    - 'Load COMPLETE file {project-root}/_kbuild/_memory/{sidecar-folder}/instructions.md'
    - 'ONLY read/write files in {project-root}/_kbuild/_memory/{sidecar-folder}/'
  ...
  menu:
    ...
    - trigger: SM or fuzzy match on save
      action: 'Update {project-root}/_kbuild/_memory/{sidecar-folder}/memories.md with insights'
...
# Read-only knowledge
- 'Load from {project-root}/_kbuild/_memory/{sidecar-folder}/knowledge/ but NEVER modify'
- 'Write ONLY to {project-root}/_kbuild/_memory/{sidecar-folder}/memories.md'
...
- Proper `{project-root}/_kbuild/_memory/` path format
...
- [ ] **ALL paths use: `{project-root}/_kbuild/_memory/{sidecar-folder}/...`**
