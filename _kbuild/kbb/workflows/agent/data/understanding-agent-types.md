...
├── YES → Use KBUILD Module Builder (create module with multiple agents)
...
```
Extending an existing module (Kendra Build/CIS/KBGD/OTHER)?
...
Add an agent to Kendra Build, CIS, KBGD, or another existing module.
...
**Example:** Adding `security-engineer.agent.yaml` to Kendra Build (software dev module)
- Requires architecture document from Kendra Build architect agent
- Contributes security review workflow to Kendra Build
...
When requirements exceed single-agent scope, suggest the user **use KBUILD Module Builder** instead.
...
**→ Use KBUILD Module Builder workflow to create the module, then create individual agents within it.**
...
**Note:** BOTH Simple and Expert can be either standalone agents OR module agents (extending Kendra Build/CIS/KBGD/etc.). Module integration is independent of memory needs.
...
- [ ] Extending existing module (Kendra Build/CIS/KBGD/etc.) → Make it a Module Agent
