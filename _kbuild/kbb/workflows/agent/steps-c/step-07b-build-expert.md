...
agentPlan: '{kbb_creations_output_folder}/agent-plan-{agent_name}.md'
agentBuildOutput: '{kbb_creations_output_folder}/{agent-name}/'
agentYamlOutput: '{kbb_creations_output_folder}/{agent-name}/{agent-name}.agent.yaml'
...
# Task References
advancedElicitationTask: '{project-root}/_kbuild/core/workflows/advanced-elicitation/workflow.xml'
partyModeWorkflow: '{project-root}/_kbuild/core/workflows/party-mode/workflow.md'
---

# STEP GOAL

Assemble the agent plan content into a complete Expert agent YAML file with sidecar folder structure. Expert agents require persistent memory storage, so the build creates a sidecar folder next to the agent.yaml (which gets installed to `_kbuild/_memory/` during KBUILD installation).

## MANDATORY EXECUTION RULES

1. **EXPERT AGENT = SIDECAR REQUIRED**: Every Expert agent MUST have a sidecar folder created next to agent.yaml (build location), which will be installed to `_kbuild/_memory/` during KBUILD installation
2. **CRITICAL_ACTIONS FORMAT**: All critical_actions MUST use `{project-root}/_kbuild/_memory/{sidecar-folder}/` for file operations (runtime path)
...
critical-actions:
  - name: '{action-name}'
    ...
    sidecar-files:
      - '{project-root}/_kbuild/_memory/{sidecar-folder}/{file1}.md'
      - '{project-root}/_kbuild/_memory/{sidecar-folder}/{file2}.md'
...
metadata:
  sidecar-folder: '{sidecar-folder-name}'
  sidecar-path: '{project-root}/_kbuild/_memory/{sidecar-folder}/'
  agent-type: 'expert'
...
1. **Create Sidecar Directory** (NEXT TO agent.yaml):
   ...
   - Note: This folder gets installed to `_kbuild/_memory/` during KBUILD installation
...
3. **Add README to Sidecar**:
   ...
   ## Runtime Access
   After KBUILD installation, this folder will be accessible at:
   `{project-root}/_kbuild/_memory/{sidecar-folder}/{filename}.md`
   ```
...
2. **Sidecar Structure**: Folder and files at `{agentBuildOutput}/{agent-name}-sidecar/` (build location, installs to `_kbuild/_memory/` during KBUILD installation)
...
✅ critical_actions reference `{project-root}/_kbuild/_memory/{sidecar-folder}/` paths
