...
Validate the agent's command menu structure against KBUILD standards as defined in agentMenuPatterns.md. Append findings to validation report and auto-advance.
...
8. **Menu Link Validation (Agent Type Specific)**
   - [ ] Determine agent type from metadata:
     ...
     - Module: module property is a module code (e.g., 'kbuild', 'kbb', 'kbgd', 'kbuild')
   ...
   - [ ] For Module agents (module property is a module code):
     ...
     - Exec paths must start with `{project-root}/_kbuild/{module}/...`
