...
## Complete Example: Kendra Build Installer
...
/**
 * Kendra Build Installer
 */
async function install(options) {
  const { projectRoot, config, installedIDEs, logger } = options;

  try {
    logger.log(chalk.blue('🚀 Installing Kendra Build...'));
    ...
    if (installedIDEs && installedIDEs.length > 0) {
      logger.log(chalk.cyan(`Configuring Kendra Build for IDEs: ${installedIDEs.join(', ')}`));
    ...
    logger.log(chalk.green('✓ Kendra Build installation complete'));
    return true;
  } catch (error) {
    logger.error(chalk.red(`Error installing Kendra Build: ${error.message}`));
    return false;
  }
}
...
2. Run `kbuild install {your-module}`
