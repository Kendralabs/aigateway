# KBUILD Workflow Template

This template provides the standard structure for all KBUILD workflow files. Copy and modify this template for each new workflow you create.
...
Load and read full config from {project-root}/_kbuild/[MODULE FOLDER]/config.yaml and resolve:
...
- `[MODULE FOLDER]` → Default is `core` unless this is for another module (such as kbuild, cis, or another as directed by user)
...
- `[CONFIG_PATH]` → Path to config file (usually `kbuild/config.yaml` or `kbb/config.yaml`)
...
`{project-root}/_kbuild/kbb/reference/workflows/meal-prep-nutrition/workflow.md`
